...
const VA_API_KEY = 'M2l1MzExeHY0d2p2dWI0Nno2cDQxOlJ6RmFUaTcwRzZyOUlXdHdIa0plcHdHMFdKcDhQYWpQ';
const VA_API_URL = 'https://api.va.landing.ai/v1/tools/agentic-document-analysis';
  ...
  try {
    ...
    if (extractedData.childDetails) {
      flatData = {
        ...
        otherChild1Relationship: extractedData.otherChildren?.[0]?.relationship || '',
        otherChild2Name: extractedData.otherChildren?.[1]?.name || '',
        otherChild2Age: extractedData.otherChildren?.[1]?.age || '',
        otherChild2Relationship: extractedData.otherChildren?.[1]?.relationship || '',
        familyDoctor: extractedData.familyInfo?.familyDoctor || '',
        familyCircumstances: extractedData.familyInfo?.familyCircumstances || '',
        childLivesWith: extractedData.familyInfo?.childLivesWith || '',
        // Handle emergency contacts
        emergencyContact1Name: extractedData.emergencyContacts?.[0]?.name || '',
        emergencyContact1Address: extractedData.emergencyContacts?.[0]?.address || '',
        emergencyContact1Phone: extractedData.emergencyContacts?.[0]?.phone || '',
        emergencyContact1Relationship: extractedData.emergencyContacts?.[0]?.relationship || '',
        emergencyContact2Name: extractedData.emergencyContacts?.[1]?.name || '',
        emergencyContact2Address: extractedData.emergencyContacts?.[1]?.address || '',
        emergencyContact2Phone: extractedData.emergencyContacts?.[1]?.phone || '',
        emergencyContact2Relationship: extractedData.emergencyContacts?.[1]?.relationship || '',
  ...
  app.listen(PORT, () => {
    ...
    if (err.code === 'EADDRINUSE') {
      ...
      app.listen(parseInt(PORT) + 1, () => {
        console.log(`🚀 Server is running at http://localhost:${parseInt(PORT) + 1}`);
      });
    } else {
